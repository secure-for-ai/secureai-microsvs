...
type SessValue struct {
	Sid        int64                  `bson:"sid" json:"sid" pg:"sid"`
	Uid        int64                  `bson:"uid" json:"uid" pg:"uid"`
	Nonce      []byte                 `bson:"nonce" json:"nonce" pg:"nonce"`
	Data       map[string]interface{} `bson:"data" json:"data" pg:"data"`
	IP         net.IP                 `bson:"ip" json:"ip" pg:"ip"`
	UserAgent  string                 `bson:"userAgent" json:"userAgent" pg:"userAgent"`
	CreateTime int64                  `bson:"createTime" json:"createTime" pg:"createTime"`
	UpdateTime int64                  `bson:"updateTime" json:"updateTime" pg:"updateTime"`
	ExpireTime int64                  `bson:"expireTime" json:"expireTime" pg:"expireTime"`
}
...
func (r *RedisMongoStoreEngine) load(ctx context.Context, sess *sessions.Session) (bool, error) {
	...
	if data == nil {
		...
		if res := bytes.Compare(nonceClient, sessValue.Nonce); res == 0 {
			...
			data, err := r.Serializer.Serialize(sess)
			if err == nil {
				_, err = r.RedisClient.Set(ctx, sess.ID, data, time.Duration(r.CacheAge))
				fmt.Println("*****", err)
			}
			//fmt.Println("save session value", sess.Values, 1000)
			//fmt.Println("save session serial", data)
...
func (r *RedisMongoStoreEngine) delete(ctx context.Context, sess *sessions.Session) error {
	...
	if errCache != nil || errDB != nil {
		return util.MultiError{errCache, errDB}
	}
	return nil
}
...
func (r *RedisPGStoreEngine) load(ctx context.Context, key string) ([]byte, error) {
...
func (r *RedisPGStoreEngine) save(ctx context.Context, key string, value []byte, duration time.Duration) error {
	_, err := r.RedisClient.Set(ctx, key, value, duration)
	return err
}

func (r *RedisPGStoreEngine) delete(ctx context.Context, key string) error {
